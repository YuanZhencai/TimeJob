...
import static com.wcs.akka.showcase.FaultHandlingDocSample.CounterServiceApi.GetCurrentCount;
import static com.wcs.akka.showcase.FaultHandlingDocSample.WorkerApi.Do;
import static com.wcs.akka.showcase.FaultHandlingDocSample.WorkerApi.Start;
...
import com.wcs.akka.showcase.FaultHandlingDocSample.CounterApi.UseStorage;
import com.wcs.akka.showcase.FaultHandlingDocSample.CounterServiceApi.CurrentCount;
import com.wcs.akka.showcase.FaultHandlingDocSample.CounterServiceApi.Increment;
import com.wcs.akka.showcase.FaultHandlingDocSample.CounterServiceApi.ServiceUnavailable;
import com.wcs.akka.showcase.FaultHandlingDocSample.StorageApi.Entry;
import com.wcs.akka.showcase.FaultHandlingDocSample.StorageApi.Get;
import com.wcs.akka.showcase.FaultHandlingDocSample.StorageApi.StorageException;
import com.wcs.akka.showcase.FaultHandlingDocSample.StorageApi.Store;
import com.wcs.akka.showcase.FaultHandlingDocSample.WorkerApi.Progress;
...
//        worker.tell(Do, listener);
    ...
    /**
     * Worker performs some work when it receives the Start message. It will continuously notify the sender of the Start message
        ...
        public void onReceive(Object msg) {
            ...
            if (msg.equals(Start) && progressListener == null) {
                ...
                getContext()
                        .system()
                        .scheduler()
                        .schedule(Duration.Zero(), Duration.create(1, "second"), getSelf(), Do, getContext().dispatcher(), null);
    ...
    /**
     * Adds the value received in Increment message to a persistent counter. Replies with CurrentCount when it is asked for
     * CurrentCount. CounterService supervise Storage and Counter.
     */
    public static class CounterService extends UntypedActor {
        ...
        /**
         * The child storage is restarted in case of failure, but after 3 restarts, and still failing it will be stopped. Better
         * to back-off than continuously failing. When it has been stopped we will schedule a Reconnect after a delay. Watch the
         * child so we receive Terminated message when it has been terminated.
         */
        void initStorage() {
        ...
        void forwardOrPlaceInBacklog(Object msg) {
            ...
            if (counter == null) {
                if (backlog.size() >= MAX_BACKLOG) throw new ServiceUnavailable("CounterService not available,"
    ...
    /**
     * The in memory count variable that will send current value to the Storage, if there is any storage available at the
     * moment.
     */
    ...
    /**
     * Saves key/value pairs to persistent storage when receiving Store message. Replies with current value when receiving Get
     * message. Will throw StorageException if the underlying data store is out of order.
     */
