...
import com.ibm.filenet.helper.ce.util.CeConfigOptions;
import com.ibm.filenet.helper.ce.util.CeConfigUtils;
import com.ibm.filenet.helper.ce.util.UserContextUtils;

public class PEConfigUtils {

    private static Logger logger = Logger.getLogger(PEConfigUtils.class);
    ...
    public static void config() {
        ...
        if (logger.isDebugEnabled()) logger.debug("jaas config file = " + jaasConfigFilePath);
        System.setProperty("java.security.auth.login.config", jaasConfigFilePath);

        String peUrl = PEConfigOptions.getProcessEngineUrl();
        System.setProperty("filenet.pe.bootstrap.ceuri", peUrl);
