...
public class BPMWorkflowServices {
    ...
    public List<WorkItem> queryWorkItemsInRoster(String condition, String[] fieldNames) throws BPMException {
        ...
        try {
            workItems = WorkItemHelper.getWorkItemsFromVWRosterElements(
                    RosterHelper.getRosterElements(this.vwSession, condition), fieldNames);
        } catch (VWException e) {
    ...
    public List<WorkItem> queryWorkItems(String queueName, String condition, String[] fieldNames) throws BPMException {
        List<WorkItem> workItems = new ArrayList<WorkItem>();
        try {
            workItems = WorkItemHelper.getWorkItemsFromVWQueueElements(
                    QueueHelper.getQueueElementsByfilter(this.vwSession, queueName, condition), fieldNames);
        } catch (VWException e) {
    ...
    public WorkItem getWorkItem(String workObjectNumber, String[] fieldNames) throws BPMException {
        try {
            return WorkItemHelper.getWorkItemFromVWStepElement(
                    RosterHelper.getStepElementByWorkObjectNumber(this.vwSession, workObjectNumber), fieldNames);
        } catch (VWException e) {
    ...
    public VWWorkObject openWorkItem(String workObjectNumber) throws BPMException {
        try {
            return StepElementHelper.lockWorkObject(RosterHelper.getStepElementByWorkObjectNumber(this.vwSession,
                    workObjectNumber));
        } catch (VWException e) {
            logger.error("通过流程号获取流程对象失败！", e);
            throw new BPMException("通过流程号获取流程对象失败！", e);
    ...
    public boolean completeAllWorkItems(List<String> queueNames, String workflowNumber) throws BPMException {
        try {
            for (String queueName : queueNames) {
                VWWorkObject vwWorkObject = QueueHelper
                        .getWorkObjectByWorkflowNumber(this.vwSession, queueName, workflowNumber);
    ...
    public boolean terminalWorkItem(String workObjectNumber) throws BPMException {
        try {
            StepElementHelper.lockWorkObject(RosterHelper.getStepElementByWorkObjectNumber(this.vwSession, workObjectNumber))
                    .doDelete(true, true);
        } catch (VWException e) {
    ...
    public VWWorkflowHistory getProcessInfo(String workObjectNumber) throws BPMException {
        ...
        try {
            VWWorkObject vwWorkObject = StepElementHelper.fetchWorkObject(RosterHelper.getStepElementByWorkObjectNumber(
                    this.vwSession, workObjectNumber));
            if (vwWorkObject != null) process = vwWorkObject.fetchProcess();
            logger.debug("\t\tWork Object Id: " + vwWorkObject.getWorkObjectNumber());

            if (process != null) history = process.fetchWorkflowHistory(-2);
        } catch (VWException e) {
            logger.error("终结流程工作项失败！", e);
            throw new BPMException("终结流程工作项失败！", e);
        }
    ...
    private void setDataMap(VWWorkObject vwWorkObject, Map<String, Object> dataMap) throws VWException {
        Assert.notNull(vwWorkObject, "流程实例为NULL.");
        for (String attrName : dataMap.keySet()) {
            Object attrValue = dataMap.get(attrName);
            if (attrValue != null) vwWorkObject.setFieldValue(attrName, attrValue, true);
        }
    }
