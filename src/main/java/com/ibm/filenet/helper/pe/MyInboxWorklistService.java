...
public class MyInboxWorklistService
{
  ...
  public static List<VWStepElement> getStepElements(VWSession session, int userid, String stepName)
    throws VWException
  {
    String filter = "F_BoundUser = :Userid AND F_StepName = :stepName";
    Object[] substitutionVars = { new Integer(userid), stepName };
    int queryFlags = 3;
    return QueueHelper.getStepElementsByfilter(session, "Inbox(0)", filter, 
      substitutionVars, queryFlags);
  }

  private static List<VWStepElement> getStepElements(VWSession session, int userid)
    throws VWException
  {
