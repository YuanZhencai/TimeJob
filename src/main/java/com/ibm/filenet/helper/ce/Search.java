...
public class Search {
    ...
    public Search setRowSqlWithFullText(String className, boolean includeSubclasses, String whereClause, String orderByClause,
            int maxRecords, boolean isFullText) {
        this.sql = new SearchSQL();
        String alias = "d";
        this.sql.setSelectList(alias + ".*, cs.Rank");
        this.sql.setFromClauseInitialValue(className, alias, includeSubclasses);
        if (isFullText) this.sql.setFromClauseAdditionalJoin(JoinOperator.INNER, "ContentSearch", "cs", "d.This",
        ...
        if (maxRecords != -1) this.sql.setMaxRecords(maxRecords);
        if (this.logger.isDebugEnabled()) {
            this.logger.debug("sql: " + this.sql.toString());
        }
        return this;
    ...
    public Search setObjectSql(String className, boolean includeSubclasses, String whereClause, String orderByClause,
            int maxRecords) {
        this.sql = new SearchSQL();
        String alias = "d";
        this.sql.setSelectList(alias + ".*");
    ...
    public Search setObjectSql(String selectClause, String className, boolean includeSubclasses, String whereClause,
            String orderByClause, int maxRecords) {
        this.sql = new SearchSQL();
        String alias = "d";
    ...
    public IndependentObjectSet fetchIndependentObjectSet() {
        return this.search.fetchObjects(this.sql, Integer.valueOf(this.pageSize), this.filter,
    ...
    public List<Document> fetchRows() {
        RepositoryRowSet rowSet = this.search.fetchRows(this.sql, Integer.valueOf(this.pageSize), this.filter,
                Boolean.valueOf(this.continuable));
