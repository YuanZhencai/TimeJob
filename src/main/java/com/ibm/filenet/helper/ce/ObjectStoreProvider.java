package com.ibm.filenet.helper.ce;
...
import com.ibm.filenet.helper.ce.util.AuthenticatedObjectStore;
import com.ibm.filenet.helper.ce.util.EngineCollectionUtils;
import com.ibm.filenet.helper.ce.util.MimeTypeMap;
import com.ibm.filenet.helper.vo.HistoryDocumentVo;
import com.ibm.filenet.helper.vo.PaginationVo;
import com.ibm.filenet.helper.vo.PermissionsVo;
import com.ibm.filenet.helper.vo.RankVo;
import com.ibm.filenet.helper.vo.RetrieveResultVo;

/** 
* <p>Project: TimeJob</p> 
* <p>Title: ObjectStoreProvider.java</p> 
* <p>Description: </p> 
...
public class ObjectStoreProvider {
    ...
    private static ObjectStoreProvider osp = null;
        ...
        if (symbolicClassName.equals("Folder")) {
            ...
            for (Iterator it = properties.iterator(); it.hasNext();) {
                ...
                for (Iterator its = selfProperties.iterator(); its.hasNext();) {
                    ...
                    if ((!(childProperty.isSettable()))
                            || (!(childProperty.getPropertyName().equals(parentProperty.getPropertyName())))) continue;
    ...
    private Folder fetchFolder(String id, PropertyFilter propertyFilter) {
        try {
            ...
            if (e.getExceptionCode() == ExceptionCode.E_OBJECT_NOT_FOUND) { return null; }
            throw e;
        }
    }
    ...
    public List<Folder> searchFolders(String className, String whereClause, boolean includeSubclasses) {
        ...
        return search.setObjectSql(className, includeSubclasses, whereClause).setScope(this.os).setPropertyFilter()
                .fetchFolders();
    }

    public List<Folder> searchFolders(String className, String whereClause, boolean includeSubclasses, String orderBy) {
        Search search = new Search();
        return search.setObjectSql(className, includeSubclasses, whereClause, orderBy, -1).setScope(this.os)
                .setPropertyFilter().fetchFolders();
    }

    public RetrieveResultVo searchFolders(String className, String whereClause, boolean includeSubclasses, String orderBy,
        ...
        Search search = new Search();
        List folderList = search.setObjectSql(className, includeSubclasses, whereClause, orderBy, -1).setScope(this.os)
                .setPropertyFilter().fetchFolders();
        ...
        if (fromIndex < toIndex) folderList = folderList.subList(fromIndex, toIndex);
        else {
    ...
    public RetrieveResultVo getDocumentListFromFolder(Folder folder, boolean onlyCurrentVersion, String docSymbolicClassName,
        ...
        while (iterator.hasNext()) {
            ...
            if (docSymbolicClassName == null) {
                if (onlyCurrentVersion) {
                    if (document.get_IsCurrentVersion().booleanValue()) documentList.add(document);
                } else documentList.add(document);
        ...
        if (fromIndex < toIndex) documentList = documentList.subList(fromIndex, toIndex);
        else {
            paginationVo.setCurrentPage((paginationVo.getTotalPages() != 0) ? paginationVo.getTotalPages() : 1);
        }
        ...
        try {
            ...
            if ((subDocuments != null) && (!(subDocuments.isEmpty()))) {
                ...
                while (dit.hasNext()) {
                    ...
                    createDocument(symbolicClassName, documentTitle, propMap, contentList, includedInFolderId);
                }
    ...
    static boolean isSettable(Property property) {
        if (SessionLocator.isExecutingInServer()) { return true; }
            ...
            try {
                ...
                for (PermissionsVo perVo : pervoList) {
                    ...
                    if (perVoTitle.equals(title)) {
                        breakflag = true;
                        if ((perVo.isAllControl()) && (perVo.isUpGradeVersion()) && (perVo.isUpdatePath())
    ...
    public String createDocument(String symbolicClassName, String documentTitle, Map<String, Object> propMap,
    ...
    public String creatingDocument(String symbolicClassName, String documentTitle, String mimeType,
            Map<String, Object> propertyMap, InputStream inputStream, Folder folder) {
        ...
        if (mimeType != "") {
            ...
            int index = documentTitle.lastIndexOf(".");
            String suffix = documentTitle.substring(index + 1);
            doc.set_MimeType(MimeTypeMap.getMimetype(suffix));
        }
    ...
    public Document getReleasedDocument(String id) {
        ...
        if (versionable == null) return null;
    ...
    public List<Document> searchObjects(String className, String whereClause, boolean includeSubclasses, int maxRecords) {
        Search search = new Search();
        return search.setObjectSql(className, includeSubclasses, whereClause, null, maxRecords).setScope(this.os)
    ...
    public List<Document> searchObjects(String selectClause, String className, String whereClause, boolean includeSubclasses,
            String orderBy) {
        Search search = new Search();
        return search.setObjectSql(selectClause, className, includeSubclasses, whereClause, orderBy, -1).setScope(this.os)
                .setPropertyFilter().fetchObjects();
    }

    public List<Document> searchObjectsByFullText(String className, String whereClause, boolean includeSubclasses,
            String orderBy) {
        Search search = new Search();
        return search.setObjectSql(className, includeSubclasses, whereClause, orderBy, -1, true).setScope(this.os)
                .setPropertyFilter().fetchObjects();
    }

    public List<RankVo> searchObjectsByFullTextWithRank(String className, String whereClause, boolean includeSubclasses,
            String orderBy) {
        Search search = new Search();
        return search.setRowSqlWithFullText(className, includeSubclasses, whereClause, orderBy, -1, true).setScope(this.os)
    ...
    public RetrieveResultVo searchObjectsByFullTextWithRank(String className, String whereClause, boolean includeSubclasses,
        ...
        Search search = new Search();
        List rankVoList = search.setRowSqlWithFullText(className, includeSubclasses, whereClause, orderBy, -1, true)
                .setScope(this.os).setPropertyFilter().fetchRowsWithRank();
        ...
        if (fromIndex < toIndex) rankVoList = rankVoList.subList(fromIndex, toIndex);
        else {
            paginationVo.setCurrentPage((paginationVo.getTotalPages() != 0) ? paginationVo.getTotalPages() : 1);
        }
    ...
    public RetrieveResultVo searchObjectsByFullText(String className, String whereClause, boolean includeSubclasses,
            boolean onlyCurrentVersion, String orderBy, PaginationVo paginationVo) {
        RetrieveResultVo rrVo = new RetrieveResultVo();
        int currentPage = paginationVo.getCurrentPage();
        int docsPerPage = paginationVo.getDocsPerPage();
        ...
        Search search = new Search();
        List documentList = search.setObjectSql(className, includeSubclasses, whereClause, orderBy, -1, true).setScope(this.os)
                .setPropertyFilter().fetchObjects();
    ...
    public RetrieveResultVo searchObjects(String className, String whereClause, boolean includeSubclasses,
            boolean onlyCurrentVersion, String orderBy, PaginationVo paginationVo) {
        RetrieveResultVo rrVo = new RetrieveResultVo();
        int currentPage = paginationVo.getCurrentPage();
        int docsPerPage = paginationVo.getDocsPerPage();
        ...
        Search search = new Search();
        List documentList = search.setObjectSql(className, includeSubclasses, whereClause, orderBy, -1).setScope(this.os)
                .setPropertyFilter().fetchObjects();
    ...
    public void deleteToRecycle(Document document) {
        ...
        Folder localFolder1;
        for (Iterator localIterator = EngineCollectionUtils.c(fs, Folder.class).iterator(); localIterator.hasNext(); localFolder1 = (Folder) localIterator
                .next())
            ;
    }
    ...
    public void moveDocument(String sourceDocumentId, String sourceParentFolderId, String destParentFolderId) throws Exception {
        try {
            ...
            rel.save(RefreshMode.NO_REFRESH);
            ReferentialContainmentRelationship destFolder = fetchFolder(destParentFolderId).file(document,
    ...
    public List<Folder> getFolderListFromRootFolder() {
        ...
        for (Folder folder : EngineCollectionUtils.c(fs, Folder.class)) {
            ...
            if ((folderName.equalsIgnoreCase("Access Roles")) || (folderName.equalsIgnoreCase("Preferences"))
                    || (folderName.equalsIgnoreCase("CodeModules"))) continue;
    ...
    public List<Document> getDocumentListFromFolder(Folder folder, String symbolicClassName) {
        ...
        while (iterator.hasNext()) {
            Document document = (Document) iterator.next();
            String className = document.getClassName();
            if (symbolicClassName == null) documentList.add(document);
    ...
    public void showProperties(String className) {
        ...
        for (PropertyDescription propDesc : EngineCollectionUtils.c(pds, PropertyDescription.class)) {
            if ((propDesc.get_IsSystemGenerated().booleanValue()) || (propDesc.get_IsSystemOwned().booleanValue())) continue;
    ...
    public void deleteInstancesByQueryStringWithBatch(String documentClassName, String queryString) {
        Search search = new Search();
        IndependentObjectSet independentObjectSet = search.setObjectSql(documentClassName, true, queryString).setScope(this.os)
                .setPropertyFilter().fetchIndependentObjectSet();
    ...
    private void deleteInstancesWithBatch(UpdatingBatch ub, IndependentObjectSet independentObjectSet) {
        ...
        if (!(myObjectsIter.hasNext())) return;
        ...
        if (symbolicClassName.equals("Document")) {
            ...
            for (it = properties.iterator(); it.hasNext();) {
                ...
                for (i = 0; i < parameterMap.size(); ++i) {
                    if (parameterMap.containsKey(prop.getPropertyName())) parameterMap.put(prop.getPropertyName(),
                            prop.getObjectValue());
                }
            }
        ...
        if (symbolicClassName.equals("Document")) {
            ...
            for (it = properties.iterator(); it.hasNext();) {
                ...
                for (i = 0; i < parameterList.size(); ++i)
                    if (parameterList.get(i) == prop.getPropertyName()) valueList.set(i, prop.getObjectValue());
            }
        } else if (symbolicClassName.equals("Folder")) {
            Folder folder = fetchFolder(id);
            properties = folder.getProperties();
    ...
    public Object getProperty(String id, String parameter, String symbolicClassName) {
        ...
        if (symbolicClassName.equals("Document")) {
            ...
            for (it = properties.iterator(); it.hasNext();) {
                prop = (Property) it.next();
                if (parameter == prop.getPropertyName()) value = prop.getObjectValue();
            }
        } else if (symbolicClassName.equals("Folder")) {
            Folder folder = fetchFolder(id);
            properties = folder.getProperties();
    ...
    public void docLinkToFolder(String docId, String linkTofolderId) throws Exception {
        try {
            ...
            for (int i = 0; i < documentList.size(); ++i) {
                compareString = ((Document) documentList.get(i)).get_Id().toString();

                if (compareString.substring(1, compareString.length() - 1).equalsIgnoreCase(docId)) { return; }
            }
            Document document = fetchDocument(docId);
    ...
    public void publishDocument(Document document, String desFolderId, String publishName, String TemplateId) throws Exception {
        try {
            ...
            String publishOptions = "<?xml version='1.0'?><publishoptions><publicationname>" + publishName
                    + "</publicationname>" + "<outputfolderid>" + desFolderId + "</outputfolderid>" + "</publishoptions>";
